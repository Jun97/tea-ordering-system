...
trait MenuItemService
{
  ...
  def createMenuItem(teaSessionId: Long, menuItemName: String, menuItemImagePath: Option[MultipartFile]): Map[String, Any]
  ...
  def updateMenuItem(menuItemId:Long, name: String, menuItemImagePath: Option[MultipartFile]): Map[String, Any]
  def deleteMenuItemById(menuItemId: Long): Map[String, Any]
  def deleteMenuItemImage(menuItemImagePath: String): Boolean
}
